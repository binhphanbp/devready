...
import { calculateSRS, qualityLabels, type ReviewQuality } from '@/lib/srs';
...
export function StudyMode({ cards, deckTitle, onComplete }: StudyModeProps) {
  ...
  // Answer source state
  const [answerSourceOverride, setAnswerSourceOverride] = useState<
    'sample' | 'user' | null
  >(null);
  ...
  useEffect(() => {
    const fetchLinkedData = async () => {
      ...
      const { data: questions } = await supabase
        .from('questions')
        .select(
          'id, title, sample_answer, bonus_tip, common_pitfalls, official_source, categories(name)',
        )
        .in('id', questionIds);
      ...
      if (questions) {
        ...
        questions.forEach((q: Record<string, unknown>) => {
          map[q.id as string] = {
            ...
            category_name:
              (q.categories as { name: string } | null)?.name ?? null,
          };
        });
  ...
  const hasLinkedQuestion =
    currentCard?.question_id && questionContents[currentCard.question_id];
  const hasUserAnswer =
    currentCard?.question_id && userAnswers[currentCard.question_id];
  ...
  if (isComplete) {
    return (
      <div className="flex flex-col items-center justify-center py-10 sm:py-16 text-center space-y-5 sm:space-y-6">
        <div className="text-6xl animate-in zoom-in-50 fade-in duration-500">
          🎉
        </div>
        <div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="h-1.5 rounded-full bg-muted/50 overflow-hidden">
        <div
          className="h-full rounded-full bg-gradient-to-r from-[#0066FF] to-[#00AAFF] transition-[width] duration-300"
                ...
                {hasLinkedQuestion && (hasUserAnswer || hasSampleAnswer) ? (
                  <Card className="border-border/50 bg-card/80 backdrop-blur-sm overflow-hidden">
                    ...
                    <div className="px-3 sm:px-4 pt-3 sm:pt-4 pb-0">
                      <Tabs
                        value={answerSource}
                        onValueChange={(v) =>
                          setAnswerSourceOverride(v as 'sample' | 'user')
                        }
                        className="w-full"
                      >
                  ...
                  <Card className="border-border/50 bg-card/80 backdrop-blur-sm min-h-[220px] sm:min-h-[280px]">
                    <CardContent className="p-5 sm:p-8 flex flex-col min-h-[220px] sm:min-h-[280px]">
                      <Badge
                        variant="outline"
                        className="mb-4 text-xs w-fit mx-auto"
                      >
                        Câu trả lời
                      </Badge>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            Tự viết câu trả lời giúp ghi nhớ tốt hơn 3x. Bấm để
                            bắt đầu viết.
                          </p>
                        </div>
      ...
      {showAIReview && currentCard?.question_id && (
        <AIReviewSheet
          ...
          questionTitle={
            questionContents[currentCard.question_id]?.title ||
            currentCard.front
          }
