...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      setUser(user);
      if (user) {
        // Check sessionStorage cache first to avoid extra DB query
    ...
    const {
      ...
      if (!session?.user) sessionStorage.removeItem('user_role');
    });
        ...
        >
          ...
          >
            ...
            <Link href="/" className="flex items-center gap-2.5 group shrink-0">
              ...
              >
                <Code2
                  className={cn('text-white', scrolled ? 'h-4 w-4' : 'h-5 w-5')}
                />
              </div>
