...
export default function AdminOverviewClient() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    const [
      ...
      supabase
        .from('questions')
        .select('id, difficulty, view_count, category_id, categories(name, color)'),
    ...
    (allQuestions ?? []).forEach(
      (q: { categories: { name: string; color: string }[] | { name: string; color: string } | null }) => {
    ...
    const [
      ...
      supabase
        .from('community_reviews')
        .select('id, company, role, upvote_count, comment_count, profiles:author_id(full_name)')
        .eq('is_approved', true)
    ...
    setStats({
      ...
      allProfileDates,
      monthlyGrowth,
      recentQuestions: (recentQuestions as unknown as Stats['recentQuestions']) ?? [],
  ...
  const growthData = useMemo(() => {
    ...
    if (growthRange === '1w') {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const label = start.toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        <StatsCard label="Người dùng" value={stats.totalUsers} icon={Users} color="blue" />
        <StatsCard label="Câu hỏi" value={stats.totalQuestions} icon={FileQuestion} color="green" />
        <StatsCard label="Chờ duyệt" value={stats.pendingQuestions} icon={Clock} color="orange" />
        <StatsCard label="Đóng góp" value={stats.totalReviews} icon={MessageSquare} color="purple" />
        <StatsCard label="Flashcard Decks" value={stats.totalFlashcardDecks} icon={Layers} color="blue" />
        <StatsCard label="Bookmarks" value={stats.totalBookmarks} icon={BookMarked} color="red" />
        <StatsCard label="Góp ý chờ xét" value={stats.pendingFeedback} icon={Lightbulb} color="orange" />
      </div>
        ...
        <Section delay={0.1}>
          ...
          <div className="space-y-3">
            {stats.difficultyDist.map((d) => {
              ...
              const pct = Math.round((d.count / (stats.totalQuestions || 1)) * 100);
              return (
                <div
                  key={d.difficulty}
                  className="group"
                ...
                >
                  <div className="mb-1 flex items-center justify-between text-xs">
                    <span className={cn(
                      ...
                      hoveredBar === d.difficulty ? 'text-white' : 'text-zinc-300',
                    )}>
                      {meta.label}
                    </span>
                    <span className={cn(
                      'transition-colors',
                      hoveredBar === d.difficulty ? 'text-zinc-300' : 'text-zinc-500',
                    )}>
                      {d.count} câu ({pct}%)
                    </span>
                  </div>
                  <div className="h-7 w-full overflow-hidden rounded-lg bg-white/4">
                    <motion.div
                      ...
                      style={{
                        ...
                        color: meta.color,
                        boxShadow: hoveredBar === d.difficulty ? `0 0 12px ${meta.bg}` : 'none',
                      }}
                    >
                      {d.count}
                    </motion.div>
                ...
                {stats.categoryDist.slice(0, 8).map((c) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        backgroundColor: isHover ? `${c.color}15` : 'transparent',
                      }}
                    ...
                    >
                      ...
                      />
                      <span className={cn('flex-1 truncate', isHover ? 'text-white font-medium' : 'text-zinc-300')}>
                        {c.name}
                      </span>
                      <span className={cn('tabular-nums', isHover ? 'text-zinc-200' : 'text-zinc-500')}>
                        {c.count}
                      </span>
                      <span className={cn('w-8 text-right tabular-nums', isHover ? 'text-zinc-300' : 'text-zinc-600')}>
                        {pct}%
                      </span>
                    </div>
                  );
                ...
                return (
                  <g key={i}>
                    ...
                    <text
                      ...
                      className={cn(
                        'text-[10px]',
                        isHovered ? 'fill-emerald-400 font-medium' : 'fill-zinc-600',
                      )}
                    >
                      {growthData[i].label}
                    </text>
