...
const statusConfig: Record<
  string,
  { label: string; color: string; selectColor: string }
> = {
  pending: {
    label: 'Chờ xét',
    color: 'text-yellow-400 bg-yellow-500/10',
    selectColor: 'text-yellow-400 bg-yellow-500/10',
  },
  in_review: {
    label: 'Đang xét',
    color: 'text-blue-400 bg-blue-500/10',
    selectColor: 'text-blue-400 bg-blue-500/10',
  },
  accepted: {
    label: 'Đã duyệt',
    color: 'text-emerald-400 bg-emerald-500/10',
    selectColor: 'text-emerald-400 bg-emerald-500/10',
  },
  rejected: {
    label: 'Từ chối',
    color: 'text-red-400 bg-red-500/10',
    selectColor: 'text-red-400 bg-red-500/10',
  },
  implemented: {
    label: 'Đã triển khai',
    color: 'text-teal-400 bg-teal-500/10',
    selectColor: 'text-teal-400 bg-teal-500/10',
  },
};
...
export default function AdminFeedbackClient() {
  ...
  const fetchFeedback = useCallback(async () => {
    ...
    let query = supabase
      .from('feedback')
      .select(
        'id, user_id, type, title, content, status, created_at, profiles(full_name, username, avatar_url)',
        { count: 'exact' },
      )
                ...
                items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <p
                          className="text-sm text-white max-w-45 truncate"
                          title={item.title}
                        >
                          {item.title}
                        </p>
                      ...
                      <td className="px-5 py-3">
                        <p
                          className="text-xs text-zinc-400 max-w-55 truncate"
                          title={item.content}
                        >
                          {item.content}
                        </p>
