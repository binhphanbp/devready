...
const mdComponents = {
  ...
  table: ({ children, ...p }: ComponentProps<'table'>) => (
    <div className="overflow-x-auto my-2">
      <table {...p} className="w-full text-xs border-collapse">
        {children}
      </table>
    </div>
  ),
  th: ({ children, ...p }: ComponentProps<'th'>) => (
    <th
      {...p}
      className="border border-border/30 px-2 py-1 bg-muted/60 font-semibold text-left"
    >
      {children}
    </th>
  ),
  td: ({ children, ...p }: ComponentProps<'td'>) => (
    <td {...p} className="border border-border/30 px-2 py-1">
      {children}
    </td>
  ),
};
      ...
      <AnimatePresence>
        {open && !expanded && (
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 sm:left-auto sm:bottom-6 sm:right-6 z-50 w-full sm:w-[420px] h-[85svh] sm:h-[600px] flex flex-col rounded-t-2xl sm:rounded-2xl border-t sm:border border-border/50 bg-card shadow-2xl overflow-hidden"
          >
            {/* Header */}
