...
function ActivityHeatmapSkeleton() {
  return (
    <div className="rounded-2xl border border-border/50 bg-card/50 p-5 sm:p-6 animate-pulse">
      ...
      <div className="h-[100px] rounded bg-muted/30" />
    </div>
  );
}
...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-5 sm:space-y-8 pt-6 sm:pt-8 lg:pt-0">
      ...
      {/* Activity Heatmap — hidden on mobile (heavy 371-cell grid), deferred on desktop */}
            ...
            <Link key={action.href} href={action.href}>
              <Card className="group relative overflow-hidden border-border/50 bg-card/50 transition-all duration-300 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 active:scale-[0.98] h-full">
                ...
                <CardContent className="relative p-4 sm:p-5">
                  <div
                    className={`flex h-10 w-10 items-center justify-center rounded-xl bg-linear-to-br ${action.gradient} mb-3`}
                  >
